...
/**
 * 將骨骼名稱格式化為可讀的顯示名稱
 * 例如: "robot_arm_joint_1" -> "Robot Arm Joint 1"
 */
function formatBoneName(name: string): string {
    return name
        .replace(/[_-]/g, ' ')           // 替換底線和連字號為空格
        .replace(/([a-z])([A-Z])/g, '$1 $2') // 在駝峰處加空格
 ...
 * 根據骨骼名稱猜測旋轉軸
 */
function guessRotationAxis(name: string): 'x' | 'y' | 'z' {
    const lowerName = name.toLowerCase();

    // 根據常見命名慣例猜測軸向
    if (/base|waist|turret/i.test(lowerName)) {
        return 'y'; // 底座通常繞 Y 軸旋轉
    }
    if (/shoulder|elbow|wrist.*pitch/i.test(lowerName)) {
        return 'z'; // 肩膀、肘部通常繞 Z 軸
    }
    if (/roll|twist|rotate/i.test(lowerName)) {
        return 'x'; // 滾轉通常繞 X 軸
    }
...
export function DynamicRobotArm({
    ...
    const setModelControls = useRobotArmStore((state) => state.setModelControls);
    const clearModelControls = useRobotArmStore((state) => state.clearModelControls);
    ...
    // 找到並綁定骨骼節點，並自動生成控制項配置
    useEffect(() => {
        if (!clonedScene || !model) return;

        console.log('🔧 正在掃描模型骨骼:', model.detail.name);

        const boneMap = new Map<string, THREE.Object3D>();
        const discoveredBones: Array<{ name: string; type: string; parent: string | null }> = [];
        ...
        clonedScene.traverse((child) => {
            if (child.name) {
                ...
                // 記錄骨骼類型的節點（Bone 或包含關節特徵的節點）
                const isBone = child.type === 'Bone';
                const isJointLike = /joint|bone|link|arm|shoulder|elbow|wrist|gripper|base/i.test(child.name);

                if (isBone || isJointLike) {
                    discoveredBones.push({
                        name: child.name,
                        type: child.type,
        ...
        console.log('✅ 找到', boneMap.size, '個節點，其中', discoveredBones.length, '個可能是骨骼');

        // 決定使用哪種控制項配置
        const apiControls = model.detail?.controls;
        const hasApiControls = apiControls && Array.isArray(apiControls) && apiControls.length > 0;

        if (hasApiControls) {
            // 使用 API 提供的控制項配置
            console.log('📋 使用 API 控制配置:', apiControls.length, '個控制項');
            setModelControls(apiControls);
        } else {
            // 自動從掃描到的骨骼生成控制項配置
            console.log('🔍 自動生成控制項配置，發現', discoveredBones.length, '個骨骼');

            const autoGeneratedControls = discoveredBones.map((bone, index) => ({
                id: `auto_${index}`,
                displayName: formatBoneName(bone.name),
                boneName: bone.name,
                axis: guessRotationAxis(bone.name) as 'x' | 'y' | 'z',
                minAngle: -180,
                maxAngle: 180,
                defaultAngle: 0,
            ...
            if (autoGeneratedControls.length > 0) {
                console.log('📤 發布自動生成的控制項:', autoGeneratedControls.length, '個');
                setModelControls(autoGeneratedControls);
            } else {
                console.warn('⚠️ 未發現可控制的骨骼');
            }
        }
    ...
    }, [clonedScene, model, setModelControls, clearModelControls]);
    ...
    useFrame((state, delta) => {
        ...
        if (isManualMode) {
            ...
            model.detail.controls.forEach((control) => {
                ...
                // 從 dynamicJointAngles 取得對應的角度值（優先）
                // 若無則嘗試從舊的 jointAngles 取得（向下相容）
                const dynamicAngle = dynamicJointAngles[control.id];
                const angleKey = control.id as keyof typeof jointAngles;
                const legacyAngle = jointAngles[angleKey];
                const targetAngle = dynamicAngle ?? legacyAngle;
